...
// //           ? "Unable to reach the contact server. Make sure the backend is running on http://localhost:5000 and CORS allows this frontend URL."
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setFormStatus({
        type: "error",
        message:
          "Unable to connect to the backend server. Make sure your backend is running on http://localhost:5000"
      });
    } finally {
      setIsSubmitting(false);
    }
            ...
            >
              <div className="grid gap-6">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="inline-flex justify-center rounded-full bg-primary px-6 py-3.5 text-sm font-semibold text-white transition duration-300 hover:-translate-y-1 hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isSubmitting ? "Sending..." : "Send Message"}
                </button>
